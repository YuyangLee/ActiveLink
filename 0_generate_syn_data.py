...
parser.add_argument("--lambda2", type=float, default=1.1)
parser.add_argument("--known", type=int, default=250)
parser.add_argument("--seed", type=int, default=2024)
parser.add_argument("--dump_dir", type=str, default="data/synthetic")
...
def generate_rule():
    ...
    before_shuffle = np.concatenate([
        np.random.choice(total_addition_entity + 2, 2 * total_triplets - total_addition_entity - 2, replace=True),
        np.arange(total_addition_entity + 2),
    ])
...
with open(os.path.join(args.dump_dir, args.output_rule), "w") as file:
...
known_relations = {
    (relation[0], entity_mapping[relation[1]], entity_mapping[relation[2]]) for relation in known_relations
}
...
with open(os.path.join(args.dump_dir, args.output_relation), "w") as file:
...
print(f"Relations have been written to {os.path.join(args.dump_dir, args.output_relation)}")

...
parser.add_argument('--lambda1', type=float, default=4)
parser.add_argument('--lambda2', type=float, default=1)
parser.add_argument('--known', type=int, default=1000)
...
def generate_rule():
    ...
    total_addition_entity = min(np.random.randint(0, 3), total_triplets - 1)
...
def check_mapping(rule, mapping, knowns):
    for relation, entity1, entity2 in rule:
        ...
        if (relation, real_entity1, real_entity2) not in knowns:
            return False, [(relation, mapping[entity1], mapping[entity2]) for relation, entity1, entity2 in rule]
    return True, [(relation, mapping[entity1], mapping[entity2]) for relation, entity1, entity2 in rule]
...
def try_mapping(rule, knowns, mapping, number_pseudo_entities, rule_index, idx=0):
    ...
    for entity in range(number):
        ...
        if idx == 1 and (rule_index, mapping[0], entity) in knowns:
            continue
        mapping[idx] = entity
        answer, used_relation = try_mapping(rule, knowns, mapping, number_pseudo_entities, rule_index, idx + 1)
        if answer:
            return answer, used_relation
    return False, used_relation
...
while True:
    ...
    for j, (rule_index, Rule) in enumerate(tqdm(All_Rules)):
        ...
        answer, used_relation = try_mapping(Rule, known_relations, mapping, number_pseudo_entities, rule_index)
        if answer:
